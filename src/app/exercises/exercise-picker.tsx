  ...
  return (
    <div className="space-y-3">
      ...
      {/* The list is its own scroll area, so anything below it in a dialog is
          ...
          a share of the viewport, not a fixed 26rem. Use a fixed height (not
          just a cap) so the modal doesn't collapse when a search returns no
          results — a shrinking sheet re-anchors and drops the search field
          behind the on-screen keyboard. */}
      <div className="h-[min(26rem,38dvh)] overflow-y-auto space-y-2 pr-1">
        {items.map((item) => {
